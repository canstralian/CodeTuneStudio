...
class MLFineTuningApp:
    """
    MLFineTuningApp: A comprehensive application for fine-tuning machine
    learning models.

    This class integrates Flask for backend operations, Streamlit for the user
    interface, and provides robust database management, plugin loading, and
    training configuration handling. It supports dataset selection, model
    training monitoring, experiment comparison, and plugin-based extensibility.

    Attributes:
        flask_app (Flask): The Flask application instance for backend operations.
        (Other attributes are managed internally via configuration and
        initialization methods.)

    Methods:
        __init__(): Initializes the app with database, Streamlit, and plugin
            configurations.
        _configure_database(): Sets up database connection with optimized pooling
            and settings.
        _initialize_database_with_retry(max_retries=3, base_delay=1.0):
            Initializes the database with retry logic and fallback to SQLite
            if needed.
        ...
        _configure_streamlit(): Configures Streamlit page settings and applies
            custom CSS.
        _load_plugins(): Discovers and loads plugins from the plugins directory.
        setup_sidebar(): Sets up the Streamlit sidebar with plugin info and
            navigation.
        _render_navigation(): Renders navigation links in the sidebar.
        save_training_config(config, dataset): Validates and saves training
            configuration to DB.
        run(): Main method to run the Streamlit app, handling UI, validation,
            and training flow.

    Usage:
        Instantiate the class and call run() to start the application. Ensure
        environment variables like DATABASE_URL are set for proper configuration.
        Plugins should be placed in the 'plugins' directory for automatic loading.

    Raises:
        RuntimeError: If Streamlit configuration fails.
        Various exceptions during database or plugin operations, logged and handled
        gracefully.
    """
        ...
        try:
            st.set_page_config(
                ...
                menu_items={
                    ...
                    "Report a bug": (
                        "https://github.com/canstralian/CodeTuneStudio/issues"
                    ),
    ...
    def _render_navigation(self) -> None:
        ...
        st.markdown(
            ...
            - [API Reference](
                https://github.com/canstralian/CodeTuneStudio/blob/main/API.md
            )
            - [Examples](
                https://github.com/canstralian/CodeTuneStudio/tree/main/examples
            )
            ...
            st.markdown(
                ...
                <div style="background: linear-gradient(135deg, #667eea 0%,
                    ...
                    <p style="color: rgba(255, 255, 255, 0.9);
                        ...
                        margin-top: 0.5rem;
                        font-size: 1.1rem;">
                        Advanced training and optimization for machine learning
                        models
                    </p>
                </div>
                """,
