                ...
                if full_repo_path:
                    ...
                    if upload == "Yes":
                        try:
                            ...
                            st.success(f"🚀 Tokenizer successfully uploaded to {full_repo_path}")
                            st.markdown(f"[View your tokenizer on Hugging Face](https://huggingface.co/{full_repo_path})")
                    ...
                    st.warning("HuggingFace repository information is required for uploading.")
                    
            except Exception as e:
                logger.error(f"Tokenizer building error: {str(e)}")
                st.error(f"Error building tokenizer: {str(e)}")
