...
class TrainingConfigModel(BaseModel):
    ...
    include_amphigory: Optional[bool] = Field(
        default=False,
        description="Whether to include amphigory data enhancement"
    )
    amphigory_ratio: Optional[float] = Field(
        default=0.1,
        ge=0.0,
        le=0.3,
        description="Ratio of amphigory data to include"
    )
    ...
    def validate_amphigory_ratio(cls, v, values):
        ...
        if values.get('include_amphigory', False) and v is None:
            raise ValueError("amphigory_ratio is required when include_amphigory is True")
        return v
